...
;; $Id: cal.asm,v 1.1 2002/03/05 16:35:29 konst Exp $
...
set_tm_wday:
	; tm_wday = (4 + days) % 7
;	mov	eax, edi	; %edi was saved copy of 'days'
	add	eax, 4		; Jan 1, 1970 was a Thursday (4)
	mov	edx, NULL
	mov	ecx, DAYS_PER_WEEK
	div	ecx
	mov	[tm_struct.wday], edx
...
.found_mon_yday:
	cld
	dec	ecx
	mov	[tm_struct.mon], ecx

set_tm_mday:
	sub	bx, ax
	inc	ebx
	mov	[tm_struct.mday], ebx

	;; put month in top row
	mov	esi, months
	mov	ecx, [tm_struct.mon]
	inc	ecx
	rep	lodsd
	mov	edi, month_buf.month_year
	add	edi, 6		; center ((20 - 4(month) - 4(year))/2)
	stosd

	;; put year in top row
	mov	eax, [tm_struct.year]
	add	edi, 3		; end of year
	...
	mov	eax, [tm_struct.mday]
	mov	edx, NULL
	mov	ecx, DAYS_PER_WEEK
	div	ecx
	dec	edx
	mov	ecx, [tm_struct.wday]
...
tm_struct B_STRUC tm,.mday,.mon,.year,.wday,.yday,.isdst
