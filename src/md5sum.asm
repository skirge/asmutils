...
;
;0.01: 20-Sep-1999	initial release
;0.02: 24-Dec-1999	
;
;syntax: md5sum [file...]
;
;if no file is given - read stdin
;
...
.read_loop:
	...
	lea esi,[buffer]
	mov ecx,eax
	call MD5_Update		; calculate MD5 for a block of data
	jmp short .read
.next_f2:
	pusha
	lea edi,[digest]
	call MD5_Final		; finalize the MD5 digest
	_mov ecx,16
	mov esi,edi
	add edi,byte 16
.write:	lodsb
...
;
	...
	sys_print text2,34	; print the resulting MD5 chain
	call MD5_Init		; re-init MD5 engine
	mov esi,[name]
	lea ecx,[ecx+34]	; if no name write '-'
	_mov edx,2
	test esi,esi
	jz short .strfini
	push ecx
...
.strf:
	...
	inc edx
.strfini:
	sys_write STDOUT		; sys_print text1,1  (use it extended for size optimization)
	popa
	jmp .next_file
...
%define c	dword [ebp-8]		;
%define	b	dword [ebp-12]		; local vars on the stack
%define	a	dword [ebp-16]		;
...
.upd_loop:
	jz short .upd2
	lea edi,[edi+edx+(buff1-LowPart)]	; edi=buff1+t

	_mov eax,64
	sub eax,edx
	mov edx,eax
	cmp ecx,edx
	jl short .updfin	; memcpy(buff1+t,input,len)
.upd1:
	push ecx
	mov ecx,edx
...
MD5_Final:
	...
	mov edx,[edi]
	shr edx,3
	and edx,byte 0x3f		; edx=t=((LowPart>>3)&0x3f)
...
buffsize equ ($-buffer)
digest:	resd 4					; the resulting 128 bits MD5 code
text2:	resd 9				; the ascii version of the MD5 code
