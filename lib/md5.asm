...
;	call MD5_Final
;
; and it's finish  ,  digest is the resulting 16 bytes (128 bit) MD5 hash value
...
%define	c	dword [ebp-12]		; local vars on the stack
%define	d	dword [ebp-16]		;
%define calling	dword [ebp-20]		;
	pusha
	mov ebp,esp
	sub esp,20		; protect the local vars space
	cld
	lea esi,[A]
	lodsd
	mov a,eax
